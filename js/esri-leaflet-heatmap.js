  ...
  var HeatmapFeatureLayer = Esri.Layers.FeatureManager.extend({
      ...
      for (var i = features.length - 1; i >= 0; i--) {
        ...
        if(!this._active[id] && (!this.options.timeField || (this.options.timeField && this._featureWithinTimeRange(geojson)))){
          ...
          if (this.intensity !== undefined) {
            if (geojson.properties.hasOwnProperty(this.intensity)) {
              intensity = geojson.properties[this.intensity];
              latlng.alt = intensity;
            }
          }

          this.heat._latlngs.push([latlng.lat, latlng.lng, intensity]);
        }
      }
    ...
    addLayers: function(ids){
      for (var i = ids.length - 1; i >= 0; i--) {
        var id = ids[i];
        if(!this._active[id]){
          console.log("Adding point", this._cache[id]);
    ...
    removeLayers: function(ids, permanent){
      ...
      for (var latlng in this._active){
          console.log("Removing point", this._cache[id]);
